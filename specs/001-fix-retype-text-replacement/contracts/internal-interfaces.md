...
**Актуализировано**: 2026-09-06 | **Модель**: [data-model.md](../data-model.md)
...
- Клиент проверяет живой `NSWorkspace.shared.frontmostApplication` относительно `activationPID` при захвате и до/после чтений фокуса. `isFocused` требует совпадения `kAXFocusedUIElementAttribute` с сохранённым элементом и совпадения его окна. При `applicationPID != activationPID` нужен дополнительный `AXFocused == true` именно у сопоставленного поля; отсутствующий, неверно типизированный или отрицательный ответ даёт отказ. При равных PID этот дополнительный атрибут не требуется.
- Допустимы `AXTextField`, `AXTextArea`, `AXComboBox`, enabled и not secure; `supportsReplacement` дополнительно требует writable value и selection range. Неактивирующая панель не освобождается от этих условий или проверки полного значения и диапазона; специальных списков приложений и приватных API определения получателя нет.
...
- Ввод готовится до установки диапазона. Перед отправкой заново проверяются привязка к получателю, живой свидетель активации, окно, AX-элемент, дополнительный фокус неактивирующего поля, поколение, полный исходный текст и точный диапазон, включая уже существующее выделение пользователя.
